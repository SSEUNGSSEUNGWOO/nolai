...
- **단어·문장 목록**: `../tools/embed/words.yaml`(레슨 1), `passages.yaml`(레슨 2), `teach.yaml`(레슨 3), `gaps.yaml`(레슨 4), `likes.yaml`(레슨 5)을 고치고 재생성한다. 레슨 9는 `tokens.yaml` + `build_tokens.py`다. 레슨 6·7의 `pixel-art.json`과 레슨 8의 `sounds-simple.json`은 **모델이 만들지 않으므로 손으로 고쳐도 된다** -- 스키마가 격자 길이·팔레트·주파수 범위를 막는다. 좌표형은 `uv run python build_dataset.py <파일>`, 문장형은 `build_passages.py`다. **좌표·각도·유사도를 손으로 고치지 않는다**
