...
/** 훅과 예측을 지나 놀이터에 선다. */
function enterPlay(guess = "강아지 근처") {
...
describe("LessonRunner", () => {
  ...
  it("훅을 지나면 예측이 나오고, 찍으면 놀이터가 나온다", () => {
    renderRunner();
    fireEvent.click(screen.getByRole("button", { name: "궁금해!" }));
    expect(screen.getByText(/어디로 갈까/)).toBeInTheDocument();
    // 찍기 전에는 놀이터도, 넘어갈 버튼도 없다
    expect(screen.queryByTestId("word-drawer")).not.toBeInTheDocument();
