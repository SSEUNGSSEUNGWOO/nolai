...
정확도가 떨어졌고(컴퓨터에게 가르쳐주기), 편향은 아예 시연되지 않았으며(없는 건
못 찾아), 갈래를 넷 넣었더니 한 쌍이 붙었다(내 취향을 어떻게 알까). 매번 손으로
스크립트를 짜서 알아냈다.
...
검사 종류
  cluster    같은 갈래끼리 뭉치는가 (비슷한 말끼리 모여라 계열)
  classify   예시 몇 개로 나머지를 맞히는가 (컴퓨터에게 가르쳐주기 계열)
  recommend  좋아한 것과 같은 갈래를 골라주는가 (내 취향을 어떻게 알까 계열)
  search     질문의 1등이 의도한 답인가 (가장 가까운 걸 찾아줘 계열)
"""
...
THRESHOLDS = {
    "cluster": 0.80,     # 비슷한 말끼리 모여라의 카테고리 이웃 일치율
    "classify": 0.85,    # 컴퓨터에게 가르쳐주기는 16/16이었다
    "recommend": 0.85,   # 내 취향을 어떻게 알까는 27/30 = 0.90
    "search": 0.90,      # 가장 가까운 걸 찾아줘는 20/20이었다
}
